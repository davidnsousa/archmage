# Archmage - Arch Linux Post Installation Setup

Post-install bash script for Arch Linux + Xfce.

Archmage installs the Pacman wrapper and AUR helper yay, updates the system, installs selected packages from the official Arch repositeries and AUR, removes unwanted packages and costumizes the desktop environment. Requires xfce4.

Archmage was written according to my software needs and DE preferences to replicate or reset my Arch system whenever I need. Feel free to read it and fork it according to your own needs.

### Usage

```
git clone https://github.com/davidnsousa/archmage
cd archmage
sh archmage.sh
```
